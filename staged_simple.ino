...
const uint16_t injector1_map[256] PROGMEM = { /* values mapping petrol injector pulse length to opening time of injector 1 */ };
const uint16_t injector2_map[256] PROGMEM = { /* values mapping petrol injector pulse length to opening time of injector 2 */ };
...
void loop() {
  ...
  // Calculate the opening times of the injectors based on the maps and the corrections
  injector1_opening_time = pgm_read_word(&injector1_map[petrol_injector_pulse]) + correction;
  injector2_opening_time = pgm_read_word(&injector2_map[petrol_injector_pulse]) + correction;
  ...
  if (petrol_injector_rising_edge) {
    ...
    // Loop until both injectors are closed or the calculated opening times are reached
